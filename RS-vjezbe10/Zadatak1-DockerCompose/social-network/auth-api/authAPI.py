...
async def login(request):
    ...
    if not korisnik:
        return web.json_response({"authorized": False, "error":"Korisnik ne postoji"})
    if hash_data(lozinka) != korisnik["lozinka_hash"]:
        return web.json_response({"authorized": False, "error": "Neispravna lozinka!"}, status=401)
    
    return web.json_response({"authorized": True, "message": "Uspjesna prijava"}, status=200)
